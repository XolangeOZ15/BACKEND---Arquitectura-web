...
public class DetallePedidoController {
   ...
   // @PreAuthorize("hasAuthority('ADMIN')")
    ...
    @PreAuthorize("hasAuthority('admin')")
    @GetMapping("/xolangeconsulta02")
    public List<CantidadPedidosPorEstadoDTO> getCantidadPedidosPorEstado() {
        List<String[]> cantidadPedidosPorEstado = dS.CantidadPedidosPorEstado();
        List<CantidadPedidosPorEstadoDTO> resultado = new ArrayList<>();

        for (String[] columna : cantidadPedidosPorEstado) {
            CantidadPedidosPorEstadoDTO dto = new CantidadPedidosPorEstadoDTO();
            dto.setEstado(columna[0]);
            dto.setCantidadPedidos(Integer.parseInt(columna[1]));
            resultado.add(dto);
        }
        return resultado;
    }
